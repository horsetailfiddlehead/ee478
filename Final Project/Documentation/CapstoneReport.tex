...
\begin{itemize}
	\item Game.c and Game.h
		\begin{itemize}
			...
			% RYAN GO DO THIS
			\item Build Cards: This function takes two arguments, a pointer to the global data structure, and a selection integer called sel that is determined by the build cards menu in the scheduler.  A switch - case statement is run that determines what data to send over I2C to the RFID card.  If sel is a 0, a FIREDUDE will be written to the card, if sel is 1 an EARTHGUY is written to the card, if sel is a 2, a WATERMAN is written to the card, and if sel is 3 a DRPECKOL is written to the card.  The commands to write the information is sent with the sendBytes() function by constructing a special I2C packet containing a WRITE_CARD_BLOCK command, a card slot number, a rfid data block to write to, and the the monster's type, level, and movelist.  The specific settings for how the packet is created can be found in the buildCard() function of the game.c file in Appendix~\ref{appendix:examplegame}.
		\end{itemize}
...
\subsection{Presentation and Discussion of Resulta} % Ryan
...
Upon system startup, the user is treated to the main menu, from which they can choose singleplayer, multiplayer, or build cards.  The user presses the 2, and 8 keys on the keypad move up and down the menu, and presses the D key to select a menu option.  When the user selects either singleplayer or multiplayer, they are shown a second menu from which they can choose Duel Game, Clue, or two empty slots.  The only slot that shows something other than an ``In progress'' screen is the Duel game option from the Singleplayer menu.  At any time, the user can place a card on the RFID reader and press down the switch to force the back-end PIC to read the card into the system.  After doing so, the Duel Game can read that card during gameplay.  The following pictures depict the singleplayer game.

\FloatBarrier
	\subsection{Reading RFID Cards}\label{appendix:read}}
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/breadBoardedSystem}
		\caption{Breadboard of the final system}
		\label{fig:breadboard}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/inventory.jpeg}
		\caption{Build Card Inventory showing available cards}
		\label{fig:inventory}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/main_menu.jpeg}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/picture1.jpeg}
		\caption{Display Keyboard}
		\label{fig:keyboard1}
	\end{figure}	
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/picture2.jpeg}
		\caption{Screen for current game status}
		\label{fig:gameState}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/picture3.jpeg}
		\caption{Display of opponent's move}
		\label{fig:displayOpponent}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/picture4.jpeg}
		\caption{Display of Select Monster Menu}
		\label{fig:attackMenu}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{images/photos/picture5.jpeg}
		\caption{Display of Attack Menu}
		\label{fig:attackMenu1}
	\end{figure}
...
Functionality of the RFID read system can be shown in the oscilloscope screenshots.
	\FloatBarrier
	\subsection{Reading RFID Cards}\label{appendix:read}}
	\begin{figure}[H]
		\centering
...
\begin{enumerate}
	\list RFID Reader / Writer
		\begin{itemise}
			\list Reading a single/multiplayer RFID tags
			\list Overwriting current RFID tag data with new data
		\end{itemize}
	\list System Hardware (SRAM, PIC, etc.)
		\begin{itemize}
			\list i2c: Front end and Back end PICs are able to exchange data between each other
			\list RS232 / UART: Able to send commands through UART/RS232 for RFID and Xbee drivers
			\list Read/Write to SRAM: Able to read and write data to the SRAM
			\list LED lights up when a card has been read - changes color to indicate status of card reading.
		\end{itemize}
	\list Xbee Wireless Communication
		\begin{itemize}
			\list Join game: Join a game hosted by another player
			\list Host game: Broadcast ATID for other players to connect
			\list Receive data: Able to receive data concerning scores and moves between players
			\list Send data: Able to send out data concerning scores and moves between players
		\end{itemize}
	\list System Peripherals (keyboard and display)
		\begin{itemize}
			\list Correctly takes in user input through keypad
			\list Correct action happens as a result of a keypress
			\list Correctly displays data
			\list Frequently updates display as game data is modified
		\end{itemize}
	\list Game Mode
			\begin{itemize}
			\list Single player works as described in software implementation
			\list Multiplayer works as described in software implementation
			\list Build card works as defined in software implementation
		\end{itemize}
\end{enumerate}
...
%
%	\subsection{Example Game}\label{appendix:examplegame}
